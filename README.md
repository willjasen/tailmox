...
After every member is visible, run `tailmox cluster` on one existing member. Tailmox requires the cluster to be quorate, verifies a unique online Tailscale address for every configured member, creates a private timestamped backup of the shared Corosync configuration under `/var/backups/tailmox`, and asks you to type `MIGRATE` before changing every Corosync link-0 address together. Before activating the generated configuration, Tailmox requires Corosync itself to accept it with `corosync -t`; a missing validator or validation failure leaves the shared configuration unchanged. Proxmox applies the validated shared configuration cluster-wide. Membership is not recreated, but Corosync may briefly lose quorum while the members move to the Tailscale network. If the cluster already uses those Tailscale addresses, Tailmox makes no Corosync change and exits successfully.

When an existing cluster is ready, Tailmox writes `/etc/pve/tailmox/state.json`, which Proxmox replicates through its cluster filesystem. The file is non-secret metadata: schema version, Proxmox cluster name, the verified Tailmox member names and expected Tailscale IPv4 addresses, and its update time. It is an operator-visible adoption record, not a source of truth: Tailmox continues to require live Tailscale status, `corosync.conf`, and `pvecm status` before making a cluster change. Do not edit it to add a host; stage and join the host through Tailmox instead.
...
This read-only test verifies that the local Proxmox host is online with the exact `tag:tailmox` identity. It tests its own Tailscale path, ICMP behavior, and required TCP ports through its Tailscale address before performing the same network checks for the other Tailmox peers. After the network checks, it reports whether the host is standalone or already a member of a Proxmox cluster, including the existing cluster name when available. ICMP warnings are reported but never prompt for `PROCEED`, so the test and monitor can finish non-interactively. It does not install packages, change Tailscale or systemd, request certificates, create a cluster, or join one.
...
- it will then retrieve other Tailscale machines with the tag of "tailmox", check all of their Tailscale DNS names in parallel using five Tailscale path pings over three seconds plus 15 64-byte and 1280-byte ICMP packets over five seconds, and check TCP 443 and TCP 8006; at least 80% (4 of 5) of the Tailscale path pings must succeed, while ICMP is evaluated separately; ICMP replies that are missing or take longer than 50 ms produce a warning with a visible countdown and require the user to type `PROCEED` within 10 seconds, otherwise setup is cancelled, while a failed Tailscale path sample or other failed check stops the script
