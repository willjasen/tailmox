...
This read-only test reports whether the host is already a member of a Proxmox cluster, including the existing cluster name, then verifies that the local Proxmox host is online with the exact `tag:tailmox` identity. It tests its own Tailscale path, ICMP behavior, and required TCP ports through its Tailscale address before performing the same network checks for the other Tailmox peers. ICMP warnings are reported but never prompt for `PROCEED`, so the test and monitor can finish non-interactively. It does not install packages, change Tailscale or systemd, request certificates, create a cluster, or join one.
