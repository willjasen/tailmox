...
When an existing cluster is ready, Tailmox writes `/etc/pve/tailmox/state.json`, which Proxmox replicates through its cluster filesystem. The file is non-secret metadata: schema version, Proxmox cluster name, every verified Tailmox member name, expected Tailscale IPv4 address, lifecycle status (`pending` while Corosync is being migrated and `active` only after post-change verification), and its update time. It is an operator-visible adoption record, not a source of truth: Tailmox continues to require live Tailscale status, `corosync.conf`, and `pvecm status` before making a cluster change. Do not edit it to add a host; stage and join the host through Tailmox instead.
