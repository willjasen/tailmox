...
if ! grep -Fq 'id="run-test"' "$TAILMOX_WEB_ROOT/index.html" ||
    ...
    grep -Fq 'reload-terminal' "$TAILMOX_WEB_ROOT/index.html" ||
    grep -Fq 'reloadTerminalButton' "$TAILMOX_WEB_ROOT/tailmox.js" ||
    ...
    ! grep -Fq 'id="monitor-dialog-hosts"' "$TAILMOX_WEB_ROOT/index.html" ||
    ...
    ! grep -Fq 'monitorDialogIssues.replaceChildren(...failureRows)' "$TAILMOX_WEB_ROOT/tailmox.js" ||
