...
const apiPrefix=(window.location.pathname==="/monitor/health"||window.location.pathname.startsWith("/monitor/"))?"/monitor":"";const issues=document.getElementById("issues"),overall=document.getElementById("overall"),add=(title,detail,bad=false)=>{const item=document.createElement("div");item.className=`issue ${bad?"bad":""}`;item.innerHTML=`<strong>${title}</strong><span>${detail}</span>`;issues.append(item)};
