...
`test-env/prepare-proxmox-test-guest.sh` script from the `dev` branch on raw
GitHub after `network-online`, so package and service setup occurs from the
repository's shared deployment branch. The finalizer removes both `ide2` and `scsi1`
